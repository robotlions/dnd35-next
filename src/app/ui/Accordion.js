    ...
    <div className="w-full">
      {props.accordionItems.map((item, index) => (
        <div key={index}>
          ...
          >
            <div className="px-6 py-4 text-gray-600 text-base min-h-52 font-[family-name:var(--font-imFell)]">
              {item.content}
            </div>
          </div>
          <br />
    ...
    <div className="w-full">
      {props.accordionItems.map((item, index) => (
        <div key={index}>
          <button
            onClick={() => toggleAccordion(index)}
            className="flex justify-between rounded items-center w-full py-4 px-6 text-left text-amber-900 hover:bg-gradient-to-b hover:from-amber-300 hover:to-amber-200 text-lg bg-gradient-to-b from-parchment to-amber-100 shadow"
          >
            <span className="text-3xl font-black font-[family-name:var(--font-imFellSC)]">
              {item.title}
            </span>
